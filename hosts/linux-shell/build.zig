...
/// Generated by wayland-scanner from protocols/*.xml (committed for offline builds).
const wayland_gen_sources = [_][]const u8{
    "src/c/gen/ext-background-effect-v1-protocol.c",
    "src/c/gen/kde-blur-protocol.c",
};
...
pub fn build(b: *std.Build) void {
    ...
    exe_mod.addCSourceFile(.{
        .file = b.path("src/c/vela_blur.c"),
        .flags = c_flags_list.items,
    });
    for (wayland_gen_sources) |src| {
        exe_mod.addCSourceFile(.{
            .file = b.path(src),
            .flags = c_flags_list.items,
        });
