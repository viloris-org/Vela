...
| G-P2-13 | Linux Wayland session features in abstraction | `@vela/api` session + paint-plan; linux-shell probe/map | **partial** (`ShellSessionFeature`, `planMaterialPaint`, Wayland global→feature map + GDK probe + **apply** ext-background-effect / KDE blur region; snapshot-blur / full L1–L6 still open) |
